...
public static void main(String[] args) {
	...
	private static long go() {
		return IntStream.range(10, 10000)
				.mapToObj(Integer::toString)
				.filter(Problem_55::isLychrel)
				.count();
	}

	private static boolean isLychrel(String input) {
		int counter = 0;
		do {
	...
	private static String reverseAndAdd(String input) {
		BigInteger inputInteger = new BigInteger(input);
		BigInteger reversedInteger = new BigInteger(reverseNumber(input));

		return inputInteger.add(reversedInteger).toString();	
	}

	private static String reverseNumber(String input) {
		StringBuilder inputTemp = new StringBuilder(input);
		return inputTemp.reverse().toString();
	}
